...
export function EditMatchModal({ match, players, matches, onEditMatch }: EditMatchModalProps) {
	const [isOpen, setIsOpen] = useState(false);
	const [teamA1, setTeamA1] = useState<ComboboxOption | null>(
		match.teamA[0] ? { value: match.teamA[0].id, label: match.teamA[0].name } : null
	);
	const [teamA2, setTeamA2] = useState<ComboboxOption | null>(
		match.teamA[1] ? { value: match.teamA[1].id, label: match.teamA[1].name } : null
	);
	const [teamB1, setTeamB1] = useState<ComboboxOption | null>(
		match.teamB[0] ? { value: match.teamB[0].id, label: match.teamB[0].name } : null
	);
	const [teamB2, setTeamB2] = useState<ComboboxOption | null>(
		match.teamB[1] ? { value: match.teamB[1].id, label: match.teamB[1].name } : null
	);
